...
task pack -depends compile {
    ...
    $packagesConfigPath = join-path (join-path $source_dir "Brf.Lofus.Core") "packages.config"
    $dependencies = [xml](Get-Content  $packagesConfigPath)
    $dependencies | Select-Xml '//package' | %{
        if ($_.Node.id -eq 'Maxfire.Prelude.Core') {
            $preludeVersion = $_.Node.version
        }
    }

    if (-not $preludeVersion) {
        throw "Could not resolve the version of the Maxfire.Prelude.Core dependency."
    }
