...
namespace Maxfire.Core.Collections
{
	...
	public sealed class Range<T>
	{
		...
		{
			string startToken = _includesStart ? "[" : "(";
			string endToken = _includesEnd ? "]" : ")";
			return startToken + _start + "; " + _end + endToken;
		}
		...
		public Range(T start, T end, bool includeStart, bool includeEnd) :
			this(start, end, Comparer<T>.Default, includeStart, includeEnd)
		{
		}
		...
		public Range(T start, T end, IComparer<T> comparer, bool includeStart, bool includeEnd)
		{
			...
			_end = end;
			_comparer = comparer;
			_includesStart = includeStart;
			_includesEnd = includeEnd;
		}
		...
		public Range<T> ExcludeEnd()
		{
			...
			return new Range<T>(_start, _end, _comparer, _includesStart, false);
		}
		...
		public Range<T> ExcludeStart()
		{
			...
			return new Range<T>(_start, _end, _comparer, false, _includesEnd);
		}
		...
		public Range<T> IncludeEnd()
		{
			...
			return new Range<T>(_start, _end, _comparer, _includesStart, true);
		}
		...
		public Range<T> IncludeStart()
		{
			...
			return new Range<T>(_start, _end, _comparer, true, _includesEnd);
		}
		...
		{
			int lowerBound = _comparer.Compare(value, _start);
			if (lowerBound < 0 || (lowerBound == 0 && !_includesStart))
			{
				return false;
			}
			int upperBound = _comparer.Compare(value, _end);
			return upperBound < 0 || (upperBound == 0 && _includesEnd);
		}
		...
		public RangeIterator<T> Step(Func<T, T> step)
		{
			...
			bool ascending = (_comparer.Compare(_start, step(_start)) < 0);
	...
	public static class Range
	{
		...
		{
			return new Range<T>(default(T), default(T), true, false);
		}
	}
}